...
`MOJIANG_API_TOKEN` 必填；浏览器在「设置 → 访问令牌」中输入相同值。
...
这不会让 Web 服务持续以 root 运行。镜像的 `mojiang-entrypoint` 只以 root 完成三件事：

1. 初始化 `/app/data`；
2. 将镜像内的只读参考数据同步到卷；
3. 修正卷权限，然后通过 `gosu` 立即降权到 `mojiang`。
