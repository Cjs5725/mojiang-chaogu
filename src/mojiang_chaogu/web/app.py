...
用法：
    ...
    # 开发模式
    uvicorn mojiang_chaogu.web.app:create_app --factory --reload
"""
...
from mojiang_chaogu.web.routes import (
    agent,
    backtest,
    baskets,
    cache,
...
from mojiang_chaogu.web.schemas import AuthStatusOut, HealthOut
...
def _frontend_dist_candidates() -> tuple[Path, ...]:
    ...
    configured = os.environ.get("MOJIANG_WEB_DIST", "").strip()
    if configured:
...
def create_app(
    ...
    async def lifespan(app: FastAPI) -> AsyncIterator[None]:
        ...
        notifier = None
        if server_chan_key:
            from mojiang_chaogu.push import (
                JsonFileDeduper,
                ServerChanPusher,
                SignalNotifier,
            )
            from mojiang_chaogu.signals import SignalSeverity
    ...
    app = FastAPI(
        title="mojiang-chaogu API",
        description="墨匠工坊的 Web 后端",
        version=__version__,
        lifespan=lifespan,
    )
    ...
    async def pair(request: Request) -> Response:
        ...
        from mojiang_chaogu.web.security import PairResult
    ...
    def health() -> HealthOut:
        from mojiang_chaogu.web.background import get_service
