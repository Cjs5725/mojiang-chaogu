...
from mojiang_chaogu.web.deps import reload_agent_caches
from mojiang_chaogu.web.schemas import (
    SetupProviderOut,
    SetupResultOut,
    SetupSaveIn,
    SetupStatusOut,
...
def _weixin_status() -> SetupWeixinStatusOut:
    ...
    try:
        ...
        from mojiang_chaogu.channels.process import gateway_process_pid
    except Exception:
...
def setup_status(request: Request) -> SetupStatusOut:
    ...
    本地 CLI 有意忽略陈旧的 MOJIANG_API_TOKEN，cfg 可能与实际运行态不一致。
    """
    cfg = load_config()
    wx = _weixin_status()
...
def _get_pairing_manager(request: Request) -> WeixinPairingManager:
    ...
    if manager is None:
        from mojiang_chaogu.web.weixin_pairing import default_pairing_manager
