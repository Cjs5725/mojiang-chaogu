---
name: mojiang-onboard
description: Explain, safely install, connect, diagnose, or repair the mojiang-chaogu Agent-first investing toolbox, then leave it open for free-form research, Strategy Card, indicator, or monitoring exploration. Use for mojiang-chaogu setup or onboarding with Claude Code, Kimi Code, Cline, or Codex, and to assess unsupported MCP hosts without pretending they have a managed adapter.
---
...
- mojiang-chaogu is a local toolbox that the current host Agent composes; it is not another LLM or a
...
`mojiang agent detect --json` only to detect those hosts. OpenClaw, Hermes, and other MCP hosts are not
valid `--host` values today.

For an unsupported host, do not invent detection, configuration paths, Skill destinations, doctor
success, or an installation command. Explain that mojiang-chaogu exposes a portable stdio MCP server
but this host is not automated by `mojiang agent` yet. Prepare a manual host-specific plan only after
...
First check whether `mojiang agent detect --json` is available. In a source checkout use
`uv run mojiang agent detect --json`; otherwise use `mojiang agent detect --json`.

If `mojiang` is missing, present an installation plan before changing the machine. Include:
...
Explain the real data boundary: keys and mojiang-chaogu databases stay on the device by default;
...
containing `mojiang agent detect` exists, prefer that released version. After installation, rerun
`mojiang agent detect --json`. If the command is still absent, state that the installed build lacks the
Agent-managed contract; do not continue by pretending an older release supports it.

Use `uv tool install --upgrade mojiang-chaogu` for a qualifying stable release. For an unreleased
commit, use the resolved SHA in
`uv tool install --force "mojiang-chaogu @ https://github.com/Cjs5725/mojiang-chaogu/archive/<SHA>.tar.gz"`.
...
1. Run `mojiang agent detect --json`.
...
4. Run:
   ...
   ```text
   mojiang agent plan --host HOST --profile PROFILE --json
   ```

5. Translate the plan into a compact human summary: configuration target, the six bundled Skills
   (mojiang-onboard, mojiang-research, mojiang-strategy, market-watch-loop for continuous intraday
...
- all six bundled Skills (mojiang-onboard, mojiang-research, mojiang-strategy, market-watch-loop,
...
`mojiang agent repair --host HOST --json` to show a repair proposal. Apply only a
...
- For a market question, use `mojiang-research` and the matching high-level `research_*` tool. Explain
  the answer, strongest evidence and timestamp, missing/stale data, fact versus inference, and one
  useful next step.
- For a method, use `mojiang-strategy` and show a faithful, readable Strategy Card. Let the user revise
...
- **Investing result** (only when the user actually ran a flow): the workflow they chose and a
  one-sentence summary of its useful result, plus where to continue (`mojiang-research` or
  `mojiang-strategy`).
