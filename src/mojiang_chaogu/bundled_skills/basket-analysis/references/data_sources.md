...
## 1. A 股大盘指数 (mojiang 无直接子命令 → 走 web)
...
## 3. 资金流 (mojiang flows + 限流兜底)

### 主路径
```bash
mojiang flows --db portfolio.db --pool watchlist pull --target today --force
```
...
### 历史资金流
- momday 默认只缓存 today, 30 天历史需要 `mojiang flows pull --target history --days 30`
...
### 限流根源
- `mojiang_chaogu.market_data.efinance_adapter.EfinanceAdapter.get_bars()` 在 sandbox 几乎总是返回 `[]` (限流)
- `mojiang_chaogu.market_data.tencent_adapter.TencentAdapter.get_bars()` 是空 stub: `def get_bars(...): return []`
...
## 7. 主题篮子 (走 mojiang food-security CLI)

```bash
mojiang food-security list --chain 上游
mojiang food-security list --subcategory 化肥-钾肥
mojiang food-security search 钾
mojiang food-security stats
```
...
3. **数据缺失标"暂不可用", 不编**: 这是 mojiang 框架的核心原则, 也是事实的边界
...
| mojiang food-security / semicon | ✅ 100% 可用 | — |
