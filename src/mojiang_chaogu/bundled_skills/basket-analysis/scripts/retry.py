...
def _find_mojiang() -> str:
    """Find the `mojiang` binary. Checks common install locations and PATH."""
    import shutil
    from pathlib import Path

    # 1. PATH first
    found = shutil.which("mojiang")
    if found:
        return found

    # 2. Common install locations (sandbox-specific)
    candidates = [
        "/workspace/.home/.local/bin/mojiang",
        "/usr/local/bin/mojiang",
        "/usr/bin/mojiang",
        Path.home() / ".local/bin/mojiang",
    ]
    for c in candidates:
        if Path(c).exists():
            return str(c)
    return "mojiang"  # fallback, will likely fail


def retry_money_flow(
    codes: list[str],
    db_path: str = "/workspace/.home/.local/share/mojiang-chaogu/portfolio.db",
    max_attempts: int = 2,
    initial_delay: float = 5.0,
) -> dict[str, bool]:
    """Best-effort money-flow pull for a list of codes via `mojiang flows --pool custom`.

    Why this exists:
    - efinance (the upstream for `mojiang flows`) rate-limits aggressively in sandboxes
    ...
    Args:
        ...
        db_path: path to portfolio.db (mojiang default)
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        print(f"  📥 money flow attempt {attempt}/{max_attempts}: pulling {len(missing)}/{len(codes)} missing codes (via {mojiang_bin})")
        missing_str = " ".join(missing)
        try:
            r = subprocess.run(
                [mojiang_bin, "flows", "--db", db_path, "--pool", "custom",
            ...
            print(f"  ⚠️ money flow attempt {attempt} failed: mojiang binary not found at {mojiang_bin}: {e}")
