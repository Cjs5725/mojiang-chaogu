...
(via mojiang CLI + web_search) and write data.json. Everything after that is
...
def _get_theme(d: dict) -> dict:
    ...
    return d.get("theme") or {
        ...
        "basket": {"size": 35, "source": "mojiang food-security"},
        "trigger_variables": [
...
def _pull_top5_flow_ts(codes) -> dict[str, list[dict]]:
    ...
    mojiang-chaogu's portfolio.db today_money_flow_cache. Returns
    ...
    """
    import sqlite3
    db_path = Path("/workspace/.home/.local/share/mojiang-chaogu/portfolio.db")
...
def render_report(d: dict[str, Any], warnings: list[str], session_id: str = "n/a") -> str:
    ...
    md.append(f"# Mojiang-chaogu · {theme_name} 主题分析报告")
    md.append(f"")
    ...
    for ex in extras:
        ...
        next_num += 1
    md.append(f"{next_num}. 重跑 `{(theme.get('basket') or {}).get('source', 'mojiang')} list` 确认 {basket_size} 只都在")
...
def render_strategy(d: dict[str, Any], session_id: str = "n/a") -> str:
    ...
    md.append(f"- [ ] **保存为可用** — 状态 `active`, 配合 `mojiang monitor` 准备监测")
    md.append(f"")
    md.append(f"---")
    md.append(f"")
...
def main() -> int:
    ...
    # Pulled from mojiang-chaogu's today_money_flow_cache (same source the agent used).
    ...
    if os.environ.get("BASKET_RETRY", "0") == "1":
        ...
        if not _codes and _basket.get("group_name"):
            try:
                import sqlite3
                _conn = sqlite3.connect(str(Path("/workspace/.home/.local/share/mojiang-chaogu/portfolio.db")))
    ...
    #   2) d["theme"]["basket"]["group_name"] (look up codes by group name in mojiang DB)
    ...
    if basket_meta.get("codes"):
        ...
        try:
            import sqlite3
            conn = sqlite3.connect(str(Path("/workspace/.home/.local/share/mojiang-chaogu/portfolio.db")))
        ...
        # Extract likely group name from "mojiang <cli>" or "mojiang watchlist <name>"
        group_name = None
        if src.startswith("mojiang "):
            group_name = src.split("mojiang ", 1)[-1]
        elif src.startswith("mojiang watchlist "):
            group_name = src.split("mojiang watchlist ", 1)[-1]
