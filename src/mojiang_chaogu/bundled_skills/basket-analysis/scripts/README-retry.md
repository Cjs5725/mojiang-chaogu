...
- **efinance** (mojiang money flow 上游) — 频繁 rate-limit, 一次只能拿到 70-80% 的股票
...
best-effort 拉一组代码的当日资金流, 自动找 `mojiang` 二进制, 用 DB cache 决定哪些还需要重试.
...
result = retry_money_flow(
    ...
    db_path="/workspace/.home/.local/share/mojiang-chaogu/portfolio.db",
    max_attempts=2,
    initial_delay=5.0,
)
...
| 数据源 | 接入方式 | 默认 | 启用 |
|---|---|---|---|
| 资金流 (mojiang) | `BASKET_RETRY=1` env | 0 (单次) | retry 2 次, 5s+10s backoff |
...
/workspace/.home/.local/share/uv/tools/mojiang-chaogu/bin/python -c "
