...
description: |
  ...
  chat-mode queries like "mojiang \"今天大盘怎么样\"" without re-running the
...
- `basket.source` (mojiang CLI / SQL group / 手动列表)
...
2. **Verify basket exists for the theme**
   - Read the theme config's `basket.source`. If `mojiang <cli>`, run `mojiang <cli> list` and `mojiang <cli> stats`. If `manual`, ensure the list is provided. If `basket_total` ≠ `len(pulled)`, may need to seed first.
   - Confirm 4 chain/subcategory (or whatever the theme config requires). If missing, run `mojiang <cli> seed` first.
...
3. **Get A-share market snapshot for `today_bj`**
   ...
   - Why: mojiang has no direct "indices" subcommand; web is the only source.
...
5. **Pull money flow for the basket**
   - Add codes to the right pool: `mojiang watchlist add-group <basket_name>` (idempotent) + `mojiang watchlist add <code> --group <basket_name>`.
   - Pull: `mojiang flows --db portfolio.db --pool watchlist pull --target today --force` (or `--pool custom --codes X Y Z` if theme has manual codes).
...
- **`retry_money_flow(codes, db_path, max_attempts=2, initial_delay=5)`** — best-effort pull. Skips already-cached codes (idempotent). Auto-finds `mojiang` binary. Returns `{code: bool}` per-code success. Does NOT raise on partial failure.
...
- `BASKET_RETRY=1` → re-runs `mojiang flows pull --force` for missing theme codes, max 2 attempts, 5s+10s backoff. Use this when first run got < 80% coverage and you want to try harder. Default is OFF (single attempt) for speed.
...
- **TencentAdapter.get_bars is a stub** in mojiang-chaogu v1.4.0 (returns `[]`). Do not use it; always call the raw API.
...
**Input**: "分析半导体板块"
**Theme loaded**: `semiconductor` (uses `mojiang semicon` template, 106 stocks, with semiconductor-specific 4 variables: 费半 SOX 同比 / 存储芯片现货价 / 国内晶圆厂稼动率 / 北向资金科技净流入)
