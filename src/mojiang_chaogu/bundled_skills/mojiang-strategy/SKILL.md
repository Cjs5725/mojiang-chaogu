...
the reading and reasoning. Use `mojiang-chaogu` MCP only for validation, local persistence, current
...
`mojiang agent plan --host <agent> --profile personal --json`, show the changed privacy scope, and
wait for approval before `mojiang agent connect`; restart the Agent after a successful connection.
