...
def render_hero(d: dict) -> str:
    ...
    <div class="hero-meta">
      {ICONS["check"]}
      <span>数据截至 {today_bj} {data_label} (北京时间) · mojiang-chaogu v1.4.0</span>
    </div>
...
def render_checklist(d: dict) -> str:
    ...
    items.append('<div class="check-item"><div class="check-num">8</div><div>重跑 <code>mojiang food-security list</code> 确认 35 只都在, <code>flows pull</code> 看 9 只限流能不能补上</div></div>')
...
def render_files(d: dict) -> str:
    ...
    <div class="file-grid">
      ...
      <div class="file-card"><div class="file-icon">{ICONS["settings"]}</div><div class="file-info"><div class="file-name">mojiang food-security CLI</div><div class="file-meta">自建 · 跟 semicon 平行 · 7 子命令</div></div></div>
