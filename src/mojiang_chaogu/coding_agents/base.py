...
from mojiang_chaogu.agent.research_tools import McpProfile, normalize_mcp_profile
from mojiang_chaogu.config import default_user_config_dir
from mojiang_chaogu.db_paths import AGENT_DB, MARKET_DB, PORTFOLIO_DB, REFERENCE_DB
from mojiang_chaogu.plugins import BUNDLED_SKILL_NAMES

SERVER_NAME = "mojiang-chaogu"
...
def connection_spec(
    ...
    *,
    ...
    Bind the MCP process to the interpreter running the current ``mojiang``
    command. A separately installed ``mojiang-mcp`` may be an older release and
    ...
    args = ["-m", "mojiang_chaogu.agent.mcp_server", "--profile", selected]
    return ConnectionSpec(
        command=command,
        args=args,
        env={
            "MOJIANG_CONFIG_DIR": str(default_user_config_dir().resolve()),
            "MOJIANG_MARKET_DB": str(MARKET_DB.resolve()),
            "MOJIANG_PORTFOLIO_DB": str(PORTFOLIO_DB.resolve()),
            "MOJIANG_AGENT_DB": str(AGENT_DB.resolve()),
            "MOJIANG_REFERENCE_DB": str(REFERENCE_DB.resolve()),
        },
...
def skill_dir(target: str, skill_name: str = "mojiang-research") -> Path:
...
def _previous_skill_record(
    ...
    if skill_name == "mojiang-research" and previous:
        return (
...
def managed_skill_records(
    ...
    if not records and previous:
        legacy = _previous_skill_record(target, previous, "mojiang-research")
        if legacy is not None:
            records["mojiang-research"] = legacy
    return records
