...
把 mojiang 的连接行合并进 ``$DSH_HOME/cordis.patch.yml``（默认 ``~/.dsh``）。

该 patch 文件归用户所有，可能同时承载其他插件行：合并时只增删 mojiang 自己的行，
...
PATCH_ROW_ID = "mcp-mojiang-chaogu"
    ...
    if version is not None and detected is None:
        return {
            ...
            "message": (
                ...
                f" mojiang 按 {TESTED_DSH_VERSION} 基线验证，格式变化时连接可能失效。"
            ),
        }
    if detected is None:
        return {
            ...
            "message": (
                ...
                f" mojiang 按 {TESTED_DSH_VERSION} 基线验证。"
            ),
        }
...
def _preserve_unknown_tag(loader: yaml.Loader, tag: str, node: yaml.Node) -> _PreservedTag:
    if not isinstance(node, yaml.ScalarNode):
        raise RuntimeError(
            f"dsh patch 中存在带未知标签 {tag} 的非标量节点，mojiang 无法安全合并；请手动添加连接行。"
        )
...
class DshAdapter:
    ...
    def _find_row(self, patches: list[Any]) -> tuple[int, int] | None:
        """返回 (操作索引, 行索引)；行身份 = mcp-client 插件 + mojiang 的 serverName。"""
