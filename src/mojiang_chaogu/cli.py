...
Command-family implementations live in :mod:`mojiang_chaogu.cli_commands`.
...
if TYPE_CHECKING:
    from mojiang_chaogu.workflow.assembly import NLRuntime
...
from mojiang_chaogu.agent.service import ChatCallbacks
from mojiang_chaogu.cli_commands.agent import *
from mojiang_chaogu.cli_commands.agent_managed import main_doctor
from mojiang_chaogu.cli_commands.cache import *
from mojiang_chaogu.cli_commands.channel import *
from mojiang_chaogu.cli_commands.connect import *
from mojiang_chaogu.cli_commands.flows import *
from mojiang_chaogu.cli_commands.memory import *
from mojiang_chaogu.cli_commands.monitor import *
from mojiang_chaogu.cli_commands.report import *
from mojiang_chaogu.cli_commands.semicon import *
from mojiang_chaogu.cli_commands.watchlist import *
from mojiang_chaogu.cli_commands.web import *
from mojiang_chaogu.cli_commands.workflow import *
from mojiang_chaogu.cli_repl import _REPL_SPARKLINE as _REPL_SPARKLINE
from mojiang_chaogu.cli_repl import render_logo as _render_logo  # noqa: F401  测试兼容 re-export
from mojiang_chaogu.cli_repl import print_workflow_result
from mojiang_chaogu.errors import friendly_error
from mojiang_chaogu.setup import main_setup
...
def _run_mojiang_repl(
    ...
    在 :mod:`mojiang_chaogu.cli_repl`，本函数只保留编排循环。
    """
    import logging
    import time
    ...
    from mojiang_chaogu.cli_repl import (
        EXIT_COMMANDS,
        ReplContext,
        ReplUI,
        run_agent_chat,
    ...
    try:
        app_version = version("mojiang-chaogu")
    ...
    # 保留原日志，用于开发者诊断。
    if not verbose:
        package_logger = logging.getLogger("mojiang_chaogu")
    ...
    while True:
        ...
        if agent is None:
            console.print(
                Panel(
                    "AI 助手尚未配置。运行 [bold]mojiang setup[/] 配置 Provider、模型和 API key。",
                    title="[yellow]需要配置[/]",
                    border_style="yellow",
                )
            )
...
def _build_dispatch() -> dict[str, tuple[str, Callable[[], object] | None]]:
    ...
    return {
        "watchlist": ("mojiang-watchlist", main_watchlist),
        "monitor": ("mojiang-monitor", main_monitor),
        "cache": ("mojiang-cache", main_cache),
        "channel": ("mojiang-channel", main_channel),
        "connect": ("mojiang-connect", main_connect),
        "setup": ("mojiang-setup", main_setup),
        "semicon": ("mojiang-semicon", main_semicon),
        "flows": ("mojiang-flows", main_flows),
        "report": ("mojiang-report", main_report),
        "agent": ("mojiang-agent", main_agent),
        "memory": ("mojiang-memory", main_memory),
        "web": ("mojiang-web", main_web),
        "tui": ("mojiang-tui", None),
        "workflow": ("mojiang-workflow", main_workflow),
        "doctor": ("mojiang-doctor", main_doctor),
    }
...
def _launch_subcommand(func: Callable[[], object] | None) -> None:
    ...
    if func is not None:
        func()
        return
    from mojiang_chaogu.tui.app import main as _tui_main
...
def _dispatch_passthrough_subcommand(
    ...
    """直接子命令（mojiang watchlist list）与 --raw 透传模式。
...
def _build_cli_toolchain() -> tuple[NLRuntime, WorkflowStore]:
    ...
    from mojiang_chaogu.workflow.assembly import build_nl_runtime
    from mojiang_chaogu.workflow.store import WorkflowStore
    ...
    if route.matched:
        ...
        if agent is None:
            print(
                ...
                "   运行 mojiang setup 配置 Provider、模型和 API key。\n"
...
def main_mojiang() -> None:
    """mojiang — 面向用户的自然语言入口。

    无参数 → 进入交互式 REPL
    带参数 → 单次自然语言查询
    <子命令> [参数] → 透传到底层 CLI（如 mojiang watchlist list）
    --raw <子命令> [参数] → 同上（向后兼容）
    """
    # 加载 .env 里的 API key（与 mojiang-agent 的 load_config、TUI bootstrap
    # 对齐——主入口漏了这步时，只配 .env 的用户会被误报「未配置 API key」）。
    # 不覆盖已有的 shell 环境变量。
    from mojiang_chaogu.config import load_runtime_env
    ...
    parser = argparse.ArgumentParser(
        prog="mojiang",
        description="墨匠工坊 - 自然语言投资助手",
        epilog=(
            "用法示例：\n"
            '  mojiang "今天怎么样"        AI 自然语言对话\n'
            "  mojiang watchlist list       结构化子命令（同 mojiang --raw watchlist list）\n"
            "  mojiang                      进入交互式 REPL\n"
            "\n"
    ...
    if unknown:
        print(
            f"⚠️ 已忽略无法识别的参数: {' '.join(unknown)}（完整用法见 mojiang --help）",
            file=sys.stderr,
        )

    # --setup 模式：运行首次配置引导
    if args.setup:
        from mojiang_chaogu.setup import run_setup_wizard

        sys.exit(0 if run_setup_wizard(offer_interface=True) else 1)

    # 安装后第一次直接运行 mojiang 时自动进入统一 onboarding；已有项目级
    # 或用户级配置时是一次无交互的快速检查。
    from mojiang_chaogu.setup import check_and_run_setup, configured_interface
    ...
    if not args.query:
        ...
        if interface == "tui":
            sys.argv = ["mojiang-tui"]
            from mojiang_chaogu.tui.app import main as _tui_main

            _tui_main()
            return
        if interface == "web":
            sys.argv = ["mojiang-web"]
            main_web()
            return
...
def main() -> int:
    ...
    p = argparse.ArgumentParser(
        prog="mojiang-chaogu",
        description="墨匠工坊 - 行情监控 / 投资陪伴",
    )
    sub = p.add_subparsers(dest="cmd", required=True)

    sub.add_parser("watchlist", help="自选股池管理").set_defaults(
        func=lambda _: _dispatch_subcommand(build_watchlist_parser(), "mojiang-watchlist")
    )
    sub.add_parser("monitor", help="行情监控").set_defaults(
        func=lambda _: _dispatch_subcommand(build_monitor_parser(), "mojiang-monitor")
    )
    sub.add_parser("cache", help="行情缓存管理").set_defaults(
        func=lambda _: _dispatch_subcommand(build_cache_parser(), "mojiang-cache")
    )
    sub.add_parser("semicon", help="半导体产业链参考库").set_defaults(
        func=lambda _: _dispatch_subcommand(build_semicon_parser(), "mojiang-semicon")
    )
    sub.add_parser("flows", help="资金流拉新 + 排行 + 监控").set_defaults(
        func=lambda _: _dispatch_subcommand(build_flows_parser(), "mojiang-flows")
    )
    sub.add_parser("report", help="报告 HTML 渲染（单日 / 索引 / 预览）").set_defaults(
        func=lambda _: _dispatch_subcommand(build_report_parser(), "mojiang-report")
    )
...
def _dispatch_subcommand(parser: argparse.ArgumentParser, prog: str) -> int:
    """把 mojiang-chaogu watchlist [args...] 转发到 watchlist parser。"""
