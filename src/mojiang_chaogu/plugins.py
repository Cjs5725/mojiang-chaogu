...
class BundledPlugin:
    """Metadata for a plugin shipped with mojiang-chaogu."""
...
BUNDLED_PLUGINS: tuple[BundledPlugin, ...] = (
    BundledPlugin(
        ...
        version="1.5.0",
        title="Mojiang Onboard",
        summary="解释产品边界、展示连接计划，并把宿主 Agent 安全带入投研工具箱。",
        category="onboarding",
        archive="skills/mojiang-onboard-v1.5.0.zip",
    ),
    BundledPlugin(
        name="mojiang-research",
        version="1.5.0",
        title="Mojiang Research",
        summary="用带时间戳和来源的行情、板块、资金流与业绩证据完成研究。",
        category="research",
        archive="skills/mojiang-research-v1.5.0.zip",
    ),
    BundledPlugin(
        name="mojiang-strategy",
        version="1.5.0",
        title="Mojiang Strategy",
        summary="把文章、研报或个人方法整理成可修订、可复用的策略卡。",
        category="strategy",
        archive="skills/mojiang-strategy-v1.5.0.zip",
    ),
    BundledPlugin(
...
def bundled_plugin(name: str) -> BundledPlugin:
    ...
    for plugin in BUNDLED_PLUGINS:
        if plugin.name == name:
            return plugin
    raise KeyError(f"未知的 mojiang-chaogu plugin: {name}")
