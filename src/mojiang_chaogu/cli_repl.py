"""mojiang REPL 的渲染与交互组件。

从 :func:`mojiang_chaogu.cli._run_mojiang_repl` 拆出的展示层：
...
from mojiang_chaogu.workflow.engine import WorkflowResult
from mojiang_chaogu.workflow.router import NLRouter, RouteResult
...
class ReplUI:
    ...
    def welcome(self) -> None:
        ...
        self.console.print(
            Panel(
                welcome_content,
                title="[bold #7c5cff]Welcome to mojiang-chaogu[/]",
    ...
    def handle_command(self, user_input: str) -> bool:
        ...
        if command == "/model":
            ...
            return True
        if command == "/tui":
            self.console.print("退出后运行 [bold]mojiang tui[/] 可进入全屏终端界面。")
            return True
        if command == "/web":
            self.console.print("退出后运行 [bold]mojiang web[/] 可启动浏览器界面。")
            return True
        return False
...
def run_agent_chat(
    ...
    with Live(
        ...
        def on_tool(name: str, args: dict[str, object]) -> None:
            ...
            from mojiang_chaogu.tui.widgets.tool_indicator import tool_display_name
