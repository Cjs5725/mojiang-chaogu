...
from mojiang_chaogu.channels import WeixinClient, WeixinCredentials, WeixinGateway, WeixinStore
...
def _run_gateway(store: WeixinStore, client: WeixinClient, *, once: bool) -> None:
    ...
    if credentials is None:
        raise WeixinApiError("尚未连接微信，请先运行 mojiang channel weixin login")
    ...
    if agent is None:
        raise WeixinApiError("尚未配置 LLM，请先运行 mojiang setup")
        ...
        if args.action == "login":
            ...
            if credentials is None:
                ...
                if pid is None:
                    ...
                    print("运行 `mojiang channel weixin start` 上线。")
                else:
