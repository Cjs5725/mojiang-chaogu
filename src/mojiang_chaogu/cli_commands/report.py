...
def cmd_report_render(args: argparse.Namespace) -> int:
    from mojiang_chaogu.report_render import parse_markdown_report, render_one

    md_path = _resolve_report_md(args.day)
    if not md_path.exists():
        print(f"❌ 找不到报告: {md_path}")
        print(f"   先跑: uv run mojiang-flows --pool {args.pool} report --day {args.day or '今天'}")
        return 1
...
def cmd_report_serve(args: argparse.Namespace) -> int:
    ...
    if not out_dir.exists():
        print(f"❌ {out_dir} 不存在，先跑 mojiang-report render")
        return 1
...
def build_report_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        ...
        epilog=(
            "example:\n"
            "  mojiang report render\n"
            "  mojiang report index\n"
            "  mojiang report serve --port 8787"
        ),
