...
from mojiang_chaogu.coding_agents.dsh import dsh_version_check
from mojiang_chaogu.version import __version__
...
def detect_payload() -> dict[str, Any]:
    ...
    return _envelope(
        ...
        unsupported_host_message=(
            "其他 MCP 宿主可使用 portable stdio server，但 mojiang agent 尚不能替它管理配置、"
    ...
    if not candidates:
        raise AgentManagedError(
            ...
            (
                ...
                "但 mojiang agent 不会假装知道其配置和 Skill 目录。"
            ),
...
def plan_payload(host: str, profile: str) -> dict[str, Any]:
    ...
    return _envelope(
        ...
        next_command=f"mojiang agent connect --host {selected} --profile {spec.profile} --json",
    )
...
def doctor_payload(host: str, timeout_seconds: float) -> dict[str, Any]:
    ...
    checks: list[dict[str, Any]] = [
        {
            ...
            "message": "已找到 mojiang 管理的连接状态" if previous else "尚未建立托管连接",
        },
        {
    ...
    if skills_ok:
        ...
        skill_message = (
            ...
            f"建议运行 `mojiang agent repair --host {selected} --json` 查看修复方案，"
    ...
    return _envelope(
        ...
        next_actions=(
            [f"运行 `mojiang agent repair --host {selected} --json` 查看安全修复建议。"]
            if blocking
            else [
...
def repair_payload(host: str, timeout_seconds: float, *, apply: bool) -> dict[str, Any]:
    ...
    if not isinstance(previous, dict):
        return _envelope(
            ...
            next_command=f"mojiang agent plan --host {selected} --profile market-only --json",
        )
    ...
    if not apply:
        return _envelope(
            ...
            applied=False,
            fixes=[fix],
            next_command=f"mojiang agent repair --host {selected} --apply --json",
        )
...
def _parser() -> argparse.ArgumentParser:
    parser = _MachineParser(prog="mojiang agent", description="Agent-managed JSON lifecycle")
...
def main_doctor() -> NoReturn:
    try:
        parser = _MachineParser(prog="mojiang doctor", description="真实检查 Agent/MCP 连接")
