"""``mojiang workflow`` commands for custom workflow specs."""
...
from mojiang_chaogu.agent.tools import ToolRegistry
from mojiang_chaogu.cli_commands.agent import _build_agent_context, _build_llm_client
from mojiang_chaogu.cli_support import AGENT_DB, argparse
...
from mojiang_chaogu.workflow.engine import Workflow, WorkflowExecutor
...
def build_workflow_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="mojiang-workflow", description="自定义交易工作流")
...
def _cmd_compile(args: argparse.Namespace, current: WorkflowSpec | None = None) -> int:
    ...
    if compiler is None:
        print("未配置 LLM，请先运行 mojiang setup")
        return 1
