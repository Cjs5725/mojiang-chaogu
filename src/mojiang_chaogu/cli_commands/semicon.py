...
def cmd_semicon_seed(args: argparse.Namespace) -> int:
    from mojiang_chaogu.semicon import seed_store
...
def cmd_semicon_get(args: argparse.Namespace) -> int:
    from mojiang_chaogu.semicon.store import StockNotFoundError
...
def cmd_semicon_add(args: argparse.Namespace) -> int:
    from mojiang_chaogu.semicon.store import StockAlreadyExistsError
...
def build_semicon_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="mojiang-semicon",
        description="墨匠工坊 - 半导体产业链参考库（A 股，按位置/产品分组）",
        epilog=(
            "example:\n"
            "  mojiang semicon seed\n"
            "  mojiang semicon list --chain 上游\n"
            "  mojiang semicon search 存储"
        ),
