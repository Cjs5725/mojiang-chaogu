...
from mojiang_chaogu.plugins import bundled_plugin_dirs
...
def _bundled_skill_dir() -> Path:
    ...
    return Path(__file__).resolve().parents[1] / "bundled_skills" / "mojiang-research"
...
def _connect(target: str, profile: str | None, *, force: bool, skip_test: bool) -> int:
    ...
    research_skill = next(path for path in skill_paths if path.name == "mojiang-research")
    item: dict[str, Any] = {
    ...
    if skip_test:
        print(f"   已跳过连通测试；稍后可运行 `mojiang connect test {target}`。")
    else:
...
def _disconnect(target: str) -> int:
    ...
    for name in names:
        item = state["connections"].get(name)
        if not isinstance(item, dict):
            print(f"{name}: 没有由 mojiang connect 管理的连接，未修改外部配置。")
            continue
        try:
