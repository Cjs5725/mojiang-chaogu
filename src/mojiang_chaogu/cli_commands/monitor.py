...
def cmd_monitor_rules(_args: argparse.Namespace) -> int:
    """列出所有内置规则 + 默认配置。"""
    from mojiang_chaogu.signals.rules import RULES_REGISTRY  # noqa: F401

    print("📐 内置告警规则")
    print("=" * 70)
    print(f"{'rule_id':<28} {'enabled':<8} {'severity':<10} {'params'}")
    print("─" * 70)
    from mojiang_chaogu.signals.rules import default_rules
...
def build_monitor_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="mojiang-monitor",
        description="墨匠工坊 - 自选股监控",
        epilog=(
            "example:\n"
            "  mojiang monitor snapshot\n"
            "  mojiang monitor run --interval 30\n"
            "  mojiang monitor log --tail 20"
        ),
