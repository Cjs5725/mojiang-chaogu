...
# ============================================================
# mojiang-agent — LLM agent 交互式 CLI
# ============================================================


def build_agent_parser() -> argparse.ArgumentParser:
    """构建 mojiang-agent 的 argparse parser。"""
    p = argparse.ArgumentParser(
        prog="mojiang-agent",
        description="墨匠工坊 - LLM agent（交互式对话 / 单次提问）",
    )
    p.add_argument(
        "query",
        nargs="*",
...
def _run_verify(argv: list[str]) -> NoReturn:
    """mojiang-agent verify：验证到期预测（cron_verify.sh 依赖）。"""
    p = argparse.ArgumentParser(
        prog="mojiang-agent verify",
...
def _run_consolidate(argv: list[str]) -> NoReturn:
    """mojiang-agent consolidate：提炼语义知识（cron_consolidate.sh 依赖）。"""
    p = argparse.ArgumentParser(
        prog="mojiang-agent consolidate",
...
def main_agent() -> NoReturn:
    """mojiang-agent 入口：交互式 LLM 对话（带工具 + 记忆系统）。"""
    # 维护子命令在 chat 解析前拦截——否则 "verify"/"consolidate"
    # 会被当成提问内容发给 LLM。
    argv = sys.argv[1:]
    if argv and argv[0] in {"detect", "plan", "connect", "doctor", "status", "repair"}:
        from mojiang_chaogu.cli_commands.agent_managed import main_agent_managed
    ...
    # 交互式 REPL
    print("墨匠工坊 Agent — 输入问题，Ctrl+C 或输入 q 退出\n")
    try:
        while True:
            try:
