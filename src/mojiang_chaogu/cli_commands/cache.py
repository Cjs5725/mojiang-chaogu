...
def cmd_cache_snapshots(args: argparse.Namespace) -> int:
    ...
    if not rows:
        print("（无全市场快照，先 mojiang-cache warmup）")
        return 0
...
def cmd_cache_config(_args: argparse.Namespace) -> int:
    from mojiang_chaogu.cache import default_config
...
def build_cache_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="mojiang-cache",
        description="墨匠工坊 - 行情缓存管理",
        epilog=(
            "example:\n"
            "  mojiang cache warmup\n"
            "  mojiang cache refresh --code 600519\n"
            "  mojiang cache stats"
        ),
