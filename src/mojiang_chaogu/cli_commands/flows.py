...
if TYPE_CHECKING:
    from mojiang_chaogu.flows.pool import PoolSource
...
def cmd_flows_top(args: argparse.Namespace) -> int:
    ...
    print("─" * 90)
    if not rows:
        print("（无数据，先 mojiang-flows pull）")
        return 0
...
def cmd_flows_run(args: argparse.Namespace) -> int:
    ...
    from mojiang_chaogu.flows import FlowMonitor
...
def cmd_flows_report(args: argparse.Namespace) -> int:
    ...
    from mojiang_chaogu.flows import FlowReport
...
def build_flows_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="mojiang-flows",
        description="墨匠工坊 - 资金流拉新 / 排行 / 监控（按股票池）",
        epilog=(
            "example:\n"
            "  mojiang flows pull\n"
            "  mojiang flows top --direction in\n"
            "  mojiang flows run --interval 300"
        ),
