...
设计原则：
- ``mojiang setup`` 原子管理 provider / model / 对应 key
...
支持的 .env / 环境变量：
    ...
    MOJIANG_API_TOKEN   → web.api_token
    MOJIANG_CORS_ORIGINS → web.cors_origins（逗号分隔）
"""
...
from mojiang_chaogu.agent.llm import SUPPORTED_PROVIDERS as _LLM_PROVIDERS
from mojiang_chaogu.agent.llm import normalize_provider as validate_agent_provider
...
def default_user_config_dir() -> Path:
    ...
    override = os.environ.get("MOJIANG_CONFIG_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    return Path.home() / ".config" / "mojiang-chaogu"
...
def load_runtime_env() -> RuntimeEnvStatus:
    ...
    with _RUNTIME_ENV_LOCK:
        ...
        for path, values, label in (
            ...
            has_secret = any(
                value and (key.endswith("API_KEY") or key in {"MOJIANG_API_TOKEN", "SERVER_CHAN_KEY"})
...
def _apply_env_overrides(cfg: AppConfig) -> AppConfig:
    ...
    env_api_token = os.environ.get("MOJIANG_API_TOKEN")
    if env_api_token:
        cfg.web.api_token = env_api_token

    env_cors = os.environ.get("MOJIANG_CORS_ORIGINS")
...
# LLM profile 和密钥统一由 ``mojiang setup`` 管理，不在这里重复配置。
_CONFIG_TEMPLATE = """\
# mojiang-chaogu 可选高级配置
# 日常安装与模型配置请运行：mojiang setup
# 本文件不保存 Provider、模型或任何密钥。

[web]
# 远程认证密钥使用 MOJIANG_API_TOKEN 环境变量。
# cors_origins 也可由 MOJIANG_CORS_ORIGINS（逗号分隔）覆盖。
cors_origins = []
