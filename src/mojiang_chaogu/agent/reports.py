...
class AgentReportService:
    ...
    def generate_daily_report(
        ...
        两种模式：
        1. pool 模式：从 flows 缓存读资金流排行（需要先 mojiang-flows pull）
        ...
        if not data:
            return "（暂无数据，请先运行 mojiang-flows pull 拉取资金流数据）"
    ...
    def _collect_board_data(self, board_code: str, board_name: str) -> dict[str, Any]:
        ...
        from mojiang_chaogu.market_data.sector_api import fetch_sector_stocks
    ...
    def _collect_pool_data(self, pool_name: str) -> dict[str, Any]:
        """从 flows 缓存读取资金流排行（需要先 mojiang-flows pull）。"""
        from mojiang_chaogu.db_paths import MARKET_DB, PORTFOLIO_DB, REFERENCE_DB
        from mojiang_chaogu.flows.pool import build_pool
    ...
    def _build_prompt(self, data: dict[str, Any], title: str) -> str:
        ...
        from mojiang_chaogu.agent.prompt import REPORT_PROMPT_TEMPLATE
