...
from mojiang_chaogu.agent.tools.base import ToolDef, _json
from mojiang_chaogu.agent.tools.registry import is_truncated_result
from mojiang_chaogu.codes import INDEX_OR_STOCK_CODE_RE as _CODE_RE
...
class ResearchToolCatalog:
            ...
            return _json(
                {
                    ...
                    "hint": (
                        "如确需使用个人数据，先运行 mojiang agent plan --host <agent> "
    ...
    def _schedule_daily_maintenance(self) -> None:
        ...
        threading.Thread(target=_run, name="mojiang-memory-maintain", daemon=True).start()
