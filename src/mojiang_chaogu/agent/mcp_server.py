...
用法：
    ...
    # 在 Claude Desktop config.json 里配：
    {
      "mcpServers": {
        "mojiang-chaogu": {
          "command": "uv",
          "args": ["run", "--directory", "/path/to/mojiang-chaogu", "mojiang-mcp",
                   "--profile", "market-only"]
        }
      }
    }
...
MCP_INSTRUCTIONS = """
mojiang-chaogu is a local investing toolbox. The host Agent is the only reasoner; never call a second
...
def _new_server(**callbacks: Any) -> Server:
    ...
    try:
        return Server("mojiang-chaogu", instructions=MCP_INSTRUCTIONS, **callbacks)
    except TypeError:  # pragma: no cover - compatibility with older MCP 1.x builds
        return Server("mojiang-chaogu", **callbacks)
...
def _build_memory_service(
    ...
    if client is not None and embedding_model is not None:
        from mojiang_chaogu.agent.vector_search import VectorSearch
...
async def run_stdio(profile: McpProfile | str = DEFAULT_MCP_PROFILE) -> None:
    ...
    async with stdio_server() as (read_stream, write_stream):
        _log.info("mojiang-chaogu MCP server started (profile=%s)", selected_profile)
        await server.run(read_stream, write_stream, server.create_initialization_options())


def main_mcp() -> None:
    """CLI 入口。"""
    parser = argparse.ArgumentParser(prog="mojiang-mcp", description="mojiang-chaogu MCP server")
    parser.add_argument(
        "--profile",
        choices=("market-only", "personal"),
        default=os.environ.get("MOJIANG_MCP_PROFILE", DEFAULT_MCP_PROFILE),
