...
class CacheManager:
    ...
    def default(cls, db_path: Path, config: CacheConfig | None = None) -> CacheManager:
        ...
        from mojiang_chaogu.cache.adapter import CachedMarketDataAdapter
        from mojiang_chaogu.market_data import EfinanceAdapter
    ...
    def warmup_market(self) -> dict[str, Any]:
        """盘前预热：拉一次全市场。"""
        from mojiang_chaogu.cache.adapter import CachedMarketDataAdapter  # noqa: F401
    ...
    def format_freshness(self) -> str:
        ...
        if not entries:
            return "（缓存为空，先 mojiang-cache warmup）"
