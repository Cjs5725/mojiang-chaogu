...
``~/.local/share/mojiang-chaogu/``，避免数据库随当前工作目录漂移。
"""
...
def default_data_dir() -> Path:
    ...
    override = os.environ.get("MOJIANG_DATA_DIR", "").strip()
    if override:
        return Path(override).expanduser()

    cwd = Path.cwd()
    if (cwd / "pyproject.toml").is_file() and (cwd / "src" / "mojiang_chaogu").is_dir():
        return Path("data")

    xdg_data_home = os.environ.get("XDG_DATA_HOME", "").strip()
    root = Path(xdg_data_home).expanduser() if xdg_data_home else Path.home() / ".local" / "share"
    return root / "mojiang-chaogu"
...
# 行情数据（缓存 + 历史 K 线 + 资金流）
MARKET_DB: Path = _path("MOJIANG_MARKET_DB", "market.db")

# 用户数据（自选股 + 持仓）
PORTFOLIO_DB: Path = _path("MOJIANG_PORTFOLIO_DB", "portfolio.db")

# 记忆系统（对话 + 事件 + 预测 + 知识 + 向量）
AGENT_DB: Path = _path("MOJIANG_AGENT_DB", "agent.db")

# 参考库（半导体产业链 + 业绩前瞻 + 业绩实际值）
REFERENCE_DB: Path = _path("MOJIANG_REFERENCE_DB", "reference.db")
