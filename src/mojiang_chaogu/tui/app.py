...
MojiangTuiApp — 投研 Coding Agent CLI：单屏对话即界面。
...
from mojiang_chaogu.tui.services.colors import FALLBACK_BG, GITHUB_TEXTUAL_THEMES, LIGHT_BG_THEMES
...
def build_tui_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="mojiang-tui",
        description="启动 mojiang-chaogu 的沉浸式终端界面。",
    )
    parser.add_argument(
        "--version",
        action="version",
...
class _MojiangCommandProvider(Provider):
...
class MojiangTuiApp(App[None]):
    """Mojiang Chaogu TUI 主应用（单屏对话）。

    用法：
        mojiang-tui          # 命令行启动
        python -m mojiang_chaogu.tui.app
    """

    TITLE = "Mojiang Chaogu"
    CSS_PATH = "styles.tcss"

    COMMANDS: ClassVar[set[type[Provider] | Any]] = {_MojiangCommandProvider}
    ...
    _TEXTUAL_THEMES: ClassVar[dict[str, str]] = {
        ...
        "atom": "atom-one-dark",
        "github": "mojiang-github-dark",
        "github-light": "mojiang-github-light",
        "dracula": "dracula",
        "tokyo": "tokyo-night",
    }
    ...
    def on_mount(self) -> None:
        ...
        self.ui_theme = os.environ.get("MOJIANG_TUI_THEME", "dark")
        ...
        # 会话自动恢复（MOJIANG_TUI_RESUME=off 关闭；/resume 仍可用）
        if self._journal is not None and (
            os.environ.get("MOJIANG_TUI_RESUME", "").strip().lower() != "off"
        ):
    ...
    def _do_workflow(
        ...
        if result is not None:
            ...
            if not summary and getattr(result, "steps", None):
                from mojiang_chaogu.workflow.engine import _format_fallback
...
def main() -> None:
    ...
    from mojiang_chaogu.setup import check_and_run_setup
