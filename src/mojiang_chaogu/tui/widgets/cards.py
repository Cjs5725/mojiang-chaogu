...
from mojiang_chaogu.tui.services.colors import color
from mojiang_chaogu.tui.services.formatting import (
    change_arrow,
    change_color,
    format_amount,
    format_change_pct,
...
def overview_card(
    ...
    if watch_total:
        ...
        lines.append("  [dim]还没有自选股（mojiang watchlist add 600519）[/]")
    extras: list[str] = []
    if signals_count:
...
def flows_command_card(code: str, info: dict[str, Any], theme: str = "dark") -> Static:
    ...
    lines.append(f"  [dim]详细：mojiang flows show {code}[/]")
    return _card(lines, classes=f"{CARD_CLASS} flow-card")
...
def portfolio_card(summary: dict[str, Any], theme: str = "dark") -> Static:
    ...
    if positions:
        ...
        lines.append("  [dim]还没有持仓记录（mojiang portfolio add-position ...）[/]")
...
def predictions_card(
    ...
    lines.append("  [dim]详细：mojiang memory predictions[/]")
...
def memory_card(
    ...
    lines.append("  [dim]详细：mojiang memory events / mojiang memory predictions[/]")
...
def onboarding_text() -> str:
    ...
    return (
        ...
        "  [bold]让 AI 干活（约 1 分钟）[/]：\n"
        "    [reverse] uv run mojiang setup [/]  ← 交互式选 Provider / 填 Key / 验证\n"
        "    然后重启 [i]mojiang-tui[/]，即可流式对话 + 工具调用 + 策略卡确认\n"
