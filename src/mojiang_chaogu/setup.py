...
TUI（mojiang-tui）首启且无可用 API key 时自动进入本向导；
CLI 用户可随时用 ``mojiang setup``（或兼容入口 ``mojiang --setup``）手动运行：
1. 选择 LLM provider
...
from mojiang_chaogu.config import default_user_env_path, load_runtime_env
...
_MANAGED_BEGIN = "# >>> mojiang-chaogu managed configuration >>>"
_MANAGED_END = "# <<< mojiang-chaogu managed configuration <<<"
CONFIG_VERSION_ENV_KEY = "MOJIANG_CONFIG_VERSION"
INTERFACE_ENV_KEY = "MOJIANG_INTERFACE"
...
def choose_interface(input_func: InputFunc = input) -> str:
    ...
    while True:
        ...
        if answer is None:
            print("\n已选择终端助手，稍后可运行 `mojiang setup` 重新选择。")
            return "cli"
...
def validate_llm_connection(provider: str, model: str, api_key: str) -> tuple[bool, str]:
    ...
    from mojiang_chaogu.agent import llm
...
def connect_weixin() -> bool:
    ...
    from mojiang_chaogu.channels import WeixinClient, WeixinStore
    from mojiang_chaogu.cli_commands.channel import _login, _start_background_gateway
...
def refresh_running_weixin_gateway() -> bool:
    ...
    from mojiang_chaogu.channels import WeixinStore
    from mojiang_chaogu.channels.process import (
        gateway_process_pid,
...
def run_setup_wizard(
    ...
    *,
    ...
    try:
        ...
        print("   请运行 `mojiang channel weixin stop` 后再运行 `mojiang channel weixin start`。")
    ...
    if offer_weixin and not weixin_reloaded:
        ...
        if pair_weixin:
            try:
                ...
                print("   稍后运行 `mojiang channel weixin connect` 可继续扫码并上线。")
    ...
    print("\n🎉 配置完成！")
    if interface == "tui":
        print("   以后运行 `mojiang` 将打开终端对话界面。")
    elif interface == "web":
        print("   以后运行 `mojiang` 将启动浏览器界面。")
    else:
        print("   mojiang                         打开专业终端助手")
    print("   mojiang channel weixin status   查看微信助手状态")
    print("   mojiang channel weixin stop     停止后台微信助手\n")
    return True
...
def check_and_run_setup(*, offer_interface: bool = False) -> bool:
    ...
    print("   稍后也可运行 `mojiang setup` 重新配置。\n")
...
def main_setup() -> None:
    """Standalone ``mojiang setup`` / ``mojiang-setup`` entry point."""
