# mojiang-chaogu × DeepSeek Harness (dsh)

把 mojiang-chaogu 接入 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness)
（dsh，"一切皆插件" 的 Cordis agent harness）有两条路：

## 推荐：托管连接（零手改配置）

```bash
uv run mojiang connect dsh                    # 默认 market-only
uv run mojiang connect dsh --profile personal # 显式开放个人能力
uv run mojiang connect status dsh
uv run mojiang connect disconnect dsh
```
...
3. 记录连接状态，支持 `status` / `disconnect` / `mojiang agent doctor --host dsh`。
...
`mojiang-dsh.cordis.yml` 是一份带注释的参考 overlay，适合一次性验证：

```bash
dsh web --patch /absolute/path/to/integrations/dsh/mojiang-dsh.cordis.yml
```
...
mojiang 的 dsh 集成对照 **`@deepseek-ai/dsh@0.1.1-rc.2`**（npm latest，2026-08）
...
- `mojiang agent doctor --host dsh --json` 会输出 `dsh_version` 检查项：与基线一致
...
- 升级 dsh 大版本后请跑 `uv run pytest tests/test_dsh_adapter.py` 回归，通过后再
  更新 `src/mojiang_chaogu/coding_agents/dsh.py` 中的 `TESTED_DSH_VERSION` 基线。
...
- dsh 的 stdio MCP 桥会剥掉环境中疑似凭据的变量和所有 `DSH_*` 变量；mojiang 依赖的
  `MOJIANG_*` 路径变量由托管连接显式写入 `config.env`，不受影响。
- dsh 会把工具名归一化到 `[A-Za-z0-9_-]`（最长 64 字符），mojiang 的工具以
  `mcp__mojiang-chaogu__<tool>` 形式出现。
